...
| Product release | RemoteAgent 0.4.1 / local image baseline |
...
| Version | Date | Change |
|---|---|---|
| 1.4.1 | 2026-09-05 | Addressed the accepted repository-critic P1 lifecycle, lease, retention, readiness, Compose, cron, and transactional-restore findings; recorded the owner-deferred backup cadence, off-host, RPO/RTO, and CI portions and the pending deployment-readiness check. |
...
| RA-AGT-009 | P0 | Met | The resolved Compose model MUST contain exactly the runner and every declared dependency, and every dependency MUST have a finite enabled health check within the configured Compose wait budget. | Validation checks the runner and all dependency services plus non-disabled test, interval, timeout, retries, finite start period, and a conservative complete-failure envelope within the wait bound. | T/I: shared `validate_compose_document`, dependency health/budget negative fixtures, and CLI/router parity tests. Health command semantics remain part of trusted agent review. |
| RA-AGT-010 | P0 | Met | Every registered runner MUST comply with the runtime isolation, mount, label, networking, logging, and resource contract in [Docker runtime and Compose contract](docker-runtime.md). | Registration rejects root execution, writable rootfs, missing capability drop or approved security settings, missing limits, Docker socket/unsafe or reordered mounts, undeclared services, unowned resources, host ports/namespaces, and `container_name`. | T/I: the router and `remotectl` use one dependency-free resolved-model validator; exhaustive forbidden-control, runner-isolation, mount, network, volume, label, logging, and dependency tests; `remotectl validate --all` passed for the reference projects. |
...
| RA-JOB-006 | P0 | Met | Subscription-backed Codex execution and dependency provisioning MUST be serialized deployment-wide by a renewable PostgreSQL lease with fencing. | Only the current lease holder provisions/runs; renewal loss interrupts the turn; a stale holder cannot renew or release a successor's lease. | T/I/O: `lease.py`, `scheduler.py`, renewal/loss/cleanup tests, and the successful disposable local PostgreSQL contention smoke covering acquisition, takeover, fencing, and stale release. |
| RA-JOB-007 | P0 | Met | A queued job MUST cancel immediately; an active job MUST observe cancellation and terminate. Every accepted job MUST have an absolute end-to-end deadline derived from its durable creation time, with independently bounded cleanup. | Tests cover queued expiry; cancellation during lease wait, provisioning, running, and collection; deadline expiry across queue/lease/preparation/provision/run/collection; router stop and lease loss; child reaping; and cleanup timeout. | T/I: `jobs.py`, `scheduler.py`, `lease.py`, `runtime.py`, and `test_scheduler_resilience.py`. The default absolute deadline is four hours and runtime/lease cleanup is separately bounded to 60 seconds. |
...
| RA-JOB-009 | P1 | Partial | Intake MUST have approved queue bounds, admission control, and queued expiration behavior. | Load tests prove a defined maximum; excess intake returns a documented response; old queued work transitions to `expired` or another approved terminal state. | T/I: jobs older than their durable absolute deadline now transition from `queued` to `expired` before claim. Gap: no approved queue-size bound, overload response, load test, or pre-parse global request-body limiter exists. |
...
| RA-CRN-004 | P0 | Met | Each occurrence MUST be persisted before MCP submission and use a deterministic `(generation, scheduled_for_utc)` identity/idempotency key. Restart MUST safely retry an ambiguous submission, resume stored-job polling, and idempotently commit one success response. | Injected crashes before/after acceptance, polling, and response commit produce at most one router job and response for an occurrence. | T/I: recovery/task-ownership tests, unique constraints, and a real FastMCP Streamable HTTP boundary test covering bearer initialization, typed calls, ambiguous acceptance replay, and malformed payload rejection. |
...
| RA-DKR-006 | P0 | Met | Runners MUST be non-root with a read-only root filesystem, bounded tmpfs, all capabilities dropped, no-new-privileges, and PID/memory/CPU limits. The reviewed outer seccomp and AppArmor exceptions MAY exist only to run the inner Bubblewrap sandbox. | Registration enforces every setting and the no-network doctor probe proves auth denial plus workspace write behavior on the deployment host. | T/D/I: the shared resolved-Compose validator and negative contract suite enforce the stanza; `remotectl validate --all` and the reference agent definitions pass. |
...
| RA-ART-006 | P1 | Partial | The immutable store SHOULD have orphan reconciliation, and production policy SHOULD define malware scanning or quarantine where artifacts cross trust boundaries. | A safe scanner reconciles DB/filesystem drift without deleting live content; scan/quarantine state is exposed and tested. | T/I: startup and periodic no-follow reconciliation protects active jobs and durable rows, removes only stale confined orphan paths after a one-hour default grace, and retries isolated failures. Gap: no malware scan, quarantine state, or cross-boundary handling policy exists. |
...
| RA-DAT-003 | P0 | Met | Readiness MUST distinguish a fully serviceable deployment from a degraded accelerator/dashboard and MUST fail when an enabled scheduler is not running. | `/readyz` returns 503 for unavailable mandatory dependencies or any missing enabled scheduler worker and explicitly reports optional Redis, dashboard, and cron degradation. | T/I: structured readiness matrix tests cover healthy, disabled, degraded, and mandatory-failure combinations; cache backend identity and configured Redis fallback are reported. |
...
| RA-DAT-005 | P0 | Met | Retention MUST commit database intent before external deletion and retry safely after partial filesystem failure. | Tests prove artifact, job, active-conversation, tombstone, and retry behavior; eligible stale bytes remain discoverable until removed without deleting active or durable data. | T/I: startup/periodic artifact reconciliation, no-follow confined deletion, per-path retry, active/durable preservation, and conversation tombstone/job-inventory tests in `test_retention.py`. |
| RA-DAT-006 | P0 | Partial | Backup MUST capture a consistent PostgreSQL dump including cron/companion state, the complete conversation tree (accepted sources and working copies included), immutable artifact store, version manifest, and checksums while excluding ephemeral unclaimed staging, credentials, secrets, Redis, images, and caches. | A scheduled operational exercise quiesces cron before the router idle check, creates/verifies/transfers a backup, and failures restart prior router/cron services in order; restored stage rows without ephemeral bytes become failed/expired. | T/I/O: `remotectl backup create/verify`, companion reconciliation, focused CLI/unit coverage, and the successful local disposable recovery smoke verify the local format/restore boundary. Gap: no approved cadence, scheduled/off-host exercise or transfer, CI gate, RPO, or RTO exists. |
| RA-DAT-007 | P0 | Met | Restore MUST verify a strict archive allowlist and checksums before destructively replacing PostgreSQL and both runtime trees, require explicit confirmation with all core services stopped, remove application objects absent from older archives, coordinate database/filesystem rollback, and restart router before cron. | SQL is validated and fully rendered before mutation; tree predecessors remain recoverable until one transactional `public`-schema reset/restore commits; an isolated drill proves current and pre-cron restore, identity/hashes, newer-object removal, rollback branches, and startup order. | T/I/O: `remotectl restore`, focused restore/failpoint tests, and the successful disposable `smoke recovery` run verified current/pre-cron archives, continuation identity, companion/artifact hashes, newer-object removal, three rollback failpoints, router-before-cron startup, and disposable cleanup. Organizational cadence, off-host copies, and RPO/RTO remain governed separately by RA-DAT-006. |
...
| RA-OBS-003 | P0 | Met | Liveness MUST report process availability, while readiness MUST fail when mandatory request-processing components are unavailable. | Public health remains shallow; authenticated readiness returns 503 for database loss or any stopped enabled scheduler worker and explicitly distinguishes Redis, dashboard, and cron degradation. | T/I: the structured readiness matrix covers healthy, disabled, optional-degradation, database-failure, and incomplete-scheduler states; it reports the active cache backend and keeps liveness independent. |
...
| RA-TST-003 | P1 | Partial | Automated acceptance SHOULD cover the production database/cache/runtime rather than relying predominantly on SQLite and fakes. | CI exercises real PostgreSQL advisory locks/contention, Redis failure/sessions, Docker isolation and sidecars, cancellation/timeouts, retention, artifact ranges, backup/restore, and API mode where supported. | T/I/O: deterministic scheduler, readiness, retention, Compose-contract, cron recovery, and MCP-boundary suites exist. The disposable local PostgreSQL and recovery smokes pass with cleanup verified. Redis/Docker sidecar, full API-mode, and protected CI acceptance remain absent. |
...
| Scheduler enabled | `true` | When enabled, readiness requires every configured scheduler worker to be alive. |
...
| Job absolute deadline | 14,400 seconds (4 hours) | Derived from durable job creation time; queueing, lease wait, preparation, dependency provisioning, execution, and result collection all consume the same budget. |
| Runtime and lease cleanup bound | 60 seconds | Independent of the absolute deadline; runtime cleanup does not rewrite an existing terminal state, while lease cleanup preserves an original failure and prevents success if ownership cannot be released first. |
...
| Artifact orphan grace | 3,600 seconds (1 hour) | Applied during startup and periodic retention reconciliation before confined orphan paths become eligible for deletion. |
...
|---|---|
| RemoteAgent router package/API | 0.4.1 |
| RemoteAgent cron package/internal API | 0.2.1 |
...
| Queue admission | Queued jobs expire at their durable absolute deadline, but intake has no queue-size or load-shedding bound. | RA-JOB-009 | Define capacity, overload response, and load acceptance without weakening durable deadline semantics. |
...
| Storage | Confined orphan reconciliation exists, but general workspace/artifact/session growth has no enforced quota and artifacts have no malware/quarantine policy. | RA-ART-006, RA-DAT-008 | Keep the state filesystem monitored/quota-capable; add enforceable general quotas and approved cross-boundary artifact handling. |
...
| Recovery assurance | The coordinated restore and local disposable drill pass, but there is no approved cadence, off-host copy, CI gate, RPO, or RTO. | RA-DAT-006, RA-GEN-004, RA-OPS-004 | Establish recurring off-host exercises, CI authority, and approved recovery targets without treating the one local drill as ongoing assurance. |
...
| Shared resolved-Compose validator, exhaustive negative contract tests, and `remotectl validate --all` | RA-AGT-009, RA-AGT-010, RA-DKR-006 |
| Scheduler deadline, cancellation, lease-loss, and cleanup tests | RA-JOB-006, RA-JOB-007, RA-JOB-009 |
| Disposable local PostgreSQL contention, sequencing, companion-claim, and cron-response smoke | RA-JOB-006, RA-DAT-004, RA-TST-003, RA-CRN-009, RA-CRN-010 |
| Retention inventory, tombstone, orphan-grace, confinement, and retry tests | RA-ART-006, RA-CON-005, RA-CON-006, RA-DAT-005 |
| Structured liveness/readiness matrix | RA-DAT-003, RA-OBS-003 |
| Deletion tombstone and cache outage tests | RA-CON-005, RA-DAT-001 |
...
| Cron worker ownership/recovery tests and real FastMCP transport-boundary tests | RA-CRN-001 through RA-CRN-011, RA-DKR-009 |
| Coordinated restore/rollback tests and successful disposable current/pre-cron recovery smoke | RA-DAT-006, RA-DAT-007, RA-TST-003 |
